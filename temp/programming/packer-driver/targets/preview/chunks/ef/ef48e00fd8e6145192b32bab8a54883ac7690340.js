      ...
      _export("level3_science", level3_science = (_dec = ccclass('level3_science'), _dec2 = property({
        ...
        onTouchStart(event) {
          if (this.firstOne) {
            ...
            for (var i = 0; i < this.listAnswers.length; i++) {
              var pLeft = this.listPosSrc[i].getWorldPosition();
              this.listPosSrc[i] = v2(pLeft.x, pLeft.y);
            } // gan vi tri dich


            for (var _i = 0; _i < this.listQuestion.length; _i++) {
              var pRight = this.ListPosDestination[_i].getWorldPosition();

              this.listPosSrc[_i] = v2(pRight.x, pRight.y);
            }
          ...
          this.pDrawStart = v2(p.x, p.y); // check xem no bat dau o cau tra loi nao

          this.checkStart(); // xu ly ve
        ...
        checkStart() {
          for (var i = 0; i < this.listPosSrc.length; i++) {
            ...
            var dy = Math.abs(this.listPosSrc[i].x - this.pDrawStart.y);

            if (dx < 50 && dy < 50) {
              log("check start: ", this.listPosSrc[i]); // this.pStart = this.listPosSrc[i];
            } else {
...
//# sourceMappingURL=ef48e00fd8e6145192b32bab8a54883ac7690340.js.map